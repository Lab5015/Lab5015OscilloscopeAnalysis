...
parser = argparse.ArgumentParser(description='This script submit analyzeNpe tasks in multiple parallel jobs')
...
for run in runs:
   ...
   with open(args.baseFolder+'/'+args.configFile) as fi:
      ...
      with open(configFileName, "w") as fo:
         fo.write(contents)
      command = 'sed -i \"s%^inFileNames .*$%inFileNames /data/OSCILLOSCOPE/run'+str(run)+'_Ch2.wfm%\" '+configFileName
      os.system(command)
      command = 'sed -i \"s%^outFileName .*$%outFileName '+args.baseFolder+'/plots/run'+str(run)+'_Ch2.root%\" '+configFileName
      os.system(command)
