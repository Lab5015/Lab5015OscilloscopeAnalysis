...
{
	double xx = x[0];
	double func = par[0] * TMath::Gaus(xx, par[1], par[2]) + par[3] + par[4] * xx + par[5] * xx * xx;
	return func;
}
...
int main(int argc, char** argv)
{
  ...
  {
    map_reader[inFileName] = new WFMReader(inFileName,false);
  ...
  TH1F* h_charge;
  if( runType == "lowLED" )                    h_charge = new TH1F("h_charge","",  256, -1000.*VUnit, 5000.*VUnit);
  ...
  if( runType == "lowLED" )
  {
    ...
    nFound = spectrum -> Search(h_charge,2,"",0.13);
    ...
    fit_singlePe = new TF1("fit_singlePe",MultiGaussian,-1000.*VUnit,vec_peaks.back()+2.*sigma,3+2*nFound+3);
		//fit_singlePe = new TF1("fit_singlePe",MultiGaussian,0.02,0.012,3+2*nFound+3);
    fit_singlePe -> SetNpx(10000);
    fit_singlePe -> FixParameter(0,nFound);
		//fit_singlePe -> SetParameter(4, fit_0pe -> GetParameter(1));
		//fit_singlePe -> SetParameter(5, vec_peaks[1] - fit_0pe -> GetParameter(1));
    ...
    /*for(int ii = 0; ii < nFound; ++ii)
    {
      fit_singlePe -> SetParameter(6+ii*2,h_charge->GetBinContent(h_charge->FindBin(vec_peaks[ii])));
      fit_singlePe -> SetParameter(7+ii*2,sigma);
    }*/
  ...
  for(auto mapIt : map_reader)
  {
    ...
    for(int iFrame = 0; iFrame < nFrames[mapIt.first]; ++iFrame)
    {
      ...
      if( runType == "lowLED" && fabs(charge-fit_singlePe->GetParameter(1+3+1)) < fit_singlePe->GetParameter(1+3+2) )
      {
        for(unsigned int iSample = 0; iSample < wf.first.size(); ++iSample)
          p_avgWf_1pe -> Fill(wf.first.at(iSample),wf.second.at(iSample));
      }
      if( runType == "lowLED" && fabs(charge-fit_singlePe->GetParameter(1+3*3+1)) < fit_singlePe->GetParameter(1+3*3+2) )
      {
        for(unsigned int iSample = 0; iSample < wf.first.size(); ++iSample)
  ...
  // p_avgWf -> Fit(fit_avgWf, "RS+","",-40.,80.);
  //fit_avgWf -> Write();
