...
ROOTGLIBS     = $(shell root-config --glibs) -lMinuit -lTreePlayer -lSpectrum -lMathMore
...
SONAME	 =  libOscilloscopeAnalysis.so
SOFLAGS  =  -Wl,-soname,
...
SONAME	 =  libOscilloscopeAnalysis.dylib
...
$(BIN)%$(BINSuf): $(PRG)%$(PRGSuf) $(HDRS) $(LIB)$(SONAME)
	$(CPP) $(CPPFLAGS) $(GLIBS) -L$(LIB) -lOscilloscopeAnalysis -o $@ $<
